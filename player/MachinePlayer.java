...
public class MachinePlayer extends Player {
  ...
  public MachinePlayer(int color, int searchDepth) {
	...
	public double evaluate(GameBoard state){
	    ...
	    if (state.nwIdentify(this.color)){	// if win for "this" MachinePlayer
			...
			if (numchips + numoppchips <= 2){               //put chips in goals early
				...
				if (color == GameBoard.BLACK && state.cellContent(4, 7) != null){
					score += 20;
				}
				if (color == GameBoard.WHITE && state.cellContent(7, 4) != null){
					score += 20;
				}
			}
			for (Chip c : chiplist){
  ...
  public Move chooseMove() {
    Move m = chooseMove(color, -5000, 5000, depth).move;
   ...
   **/
	  ...
	  public Best chooseMove(int side, int alpha, int beta, int searchdepth){
	      ...
	      if((side == color && evaluate(board) == 5000.0) || (side == oppcolor && evaluate(board) == -5000) || searchdepth == 0){
	      	return new Best(valid_moves[0], evaluate(board));     //guaranteed a win or reached search depth
	      }
	      ...
	      if (myBest.move == null){
	      	System.out.println("De Fuq" + Arrays.toString(valid_moves));
	      	return new Best(valid_moves[0], 0);
	      }else{
	      	return myBest;
	      }
	  }
